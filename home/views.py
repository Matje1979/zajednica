...
class CepPrijava(APIView):
    ...
    def post(self, request, *args, **kwargs):
        # ulaz = Ulaz.objects.get(Ulica_i_broj=request.data['ulaz'])
        # This line is necessary because the field ulaz
        # accepts object ulaz not an address string.
        ulaz = Ulaz.objects.get(Ulica_i_broj=request.data["ulaz"])
        ...
        date = time.time()
        if (
            len(TempCepovi.objects.filter(ulaz=ulaz)) == 0
            and ulaz.cep_box_full is False
        ):
            TempCepovi.objects.create(
                foto=ImageFile(open("image.png", "rb")),
                ulaz=ulaz,
                cep_box_filled_date=date,
            )
            # serializer = CepPrijavaSerializer(data=request.data)
            # print ("Serializer", serializer, flush=True)
            # flush=True seems necessary to be able to print in console
            # that is to get server logs in pythonanywhere.
            # print (serializer, flush=True)
            # if serializer.is_valid():
            #     try:
            # print (serializer, flush=True)
...
class GradesList(APIView):
    def get(self, request):
        grade_list = []
        users_profiles = Profile.objects.filter(is_director=True).order_by(
            "-prosecna_ocena"
        )[:10]
...
class PostListView(LoginRequiredMixin, ListView):
    ...
    # for a ListView, default context object name is object_list,
    # but that can be overriden in the way above.
    # for DetailView, default context object name is 'object.


# Base html (view necessary for pwa offline mode)
...
class PostCreateView(LoginRequiredMixin, CreateView):
    ...
    # LoginRequiredMixin serves to block access for
...
class PostCreateView2(LoginRequiredMixin, CreateView):
    ...
    def get_initial(self):
        return {
            "anketa_title": self.kwargs["anketa_title"],
            "anketa_id": self.kwargs["anketa_id"],
            "title": "Anketa",
        }
            ...
            if not TempPapir.objects.filter(ulaz=ulaz):
                if request.FILES.get("foto"):
                    ...
                    TempPapir.objects.create(
                        ...
                        ulica_i_broj=ulaz.Ulica_i_broj,
                        location=ulaz.location,
                    )
...
# MessageForUpravnik, pk = self.kwargs['pk'])
#             receiver = msg.sender
...
def collectors_map(request):
    ...
    # the view sends data by default to a template
    # with the following name pattern:
    # /app_name/model_name/underscore/type_of_view(
    # list, detail... except for create view, where it expects 'form' instead)
